...
The workflow tools deliberately separate read-only snapshots from mutating
execution. Headless or non-approved runs can produce proposals, but they cannot
execute workflow mutations.
