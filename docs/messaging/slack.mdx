...
| `slack_capture_task` | Persist “add task …” / “remind me …” locally | none (writes `~/.opensre/slack_captured_tasks.jsonl`) | No |
...
The EC2 paths (`make deploy` / `make deploy-gateway`) are **Telegram-only** and
