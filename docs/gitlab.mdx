...
pass `gitlab_url` or `gitlab_token` as tool arguments; those values are
...
After connecting GitLab, include a GitLab project or file URL in your request.
OpenSRE infers the project scope from the current message and keeps it for
...
OpenSRE also checks recent conversation, `GITLAB_PROJECT_ID`, `GITLAB_REF`,
